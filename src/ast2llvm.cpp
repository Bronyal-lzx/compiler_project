...
void ast2llvmBlock(aA_codeBlockStmt b, Temp_label *con_label, Temp_label *bre_label)
{
        ...
        else if (varDeclStmt->kind == A_varDeclStmtType::A_varDefKind)
        {
            ...
            case A_varDefType::A_varDefScalarKind:
            {
                ...
                case A_dataType::A_structTypeKind:
                {
                    ...
                    // ir: store
                    emit_irs.push_back(L_Store(ast2llvmRightVal(defScalar->val), AS_Operand_Temp(temp)));
                }
                break;
    ...
    case A_codeBlockStmtType::A_ifStmtKind:
    {
        ...
        if (ifStmt->elseStmts.empty())
        {
            ...
            auto ifEndLabel = Temp_newlabel();

            // irs: cond
            ast2llvmBoolExpr(ifStmt->boolExpr, ifTrueLabel, ifEndLabel);
        ...
        else
        {
            ...
            // irs: cond
            ast2llvmBoolExpr(ifStmt->boolExpr, ifTrueLabel, ifFalseLabel);
    ...
    case A_codeBlockStmtType::A_whileStmtKind:
    {
        ...
        ast2llvmBoolExpr(whileStmt->boolExpr, whileTrueLabel, whileFalseLabel);
...
AS_operand *ast2llvmLeftVal(aA_leftVal l)
{
    ...
    case A_leftValType::A_varValKind:
    {
        ...
        else if (localVarMap.find(id) != localVarMap.end())
        {
...
AS_operand *ast2llvmBoolExpr(aA_boolExpr b, Temp_label *true_label, Temp_label *false_label)
{
    ...
    if (should_return)
    {
        auto temp_true = Temp_newtemp_int();
        auto temp_false = Temp_newtemp_int();
        auto temp_end = Temp_newtemp_int();
        auto end_label = Temp_newlabel();

        // ir: move true
        emit_irs.push_back(L_Label(true_label));
        emit_irs.push_back(L_Move(AS_Operand_Const(1), AS_Operand_Temp(temp_true)));
        emit_irs.push_back(L_Jump(end_label));

        // ir: move false
        emit_irs.push_back(L_Label(false_label));
        emit_irs.push_back(L_Move(AS_Operand_Const(0), AS_Operand_Temp(temp_false)));
        emit_irs.push_back(L_Jump(end_label));

        // ir: end
        emit_irs.push_back(L_Label(end_label));
        emit_irs.push_back(L_Phi(AS_Operand_Temp(temp_end), {{AS_Operand_Temp(temp_true), true_label}, {AS_Operand_Temp(temp_false), false_label}}));

        return AS_Operand_Temp(temp_end);
    }
